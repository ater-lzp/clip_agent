...
- `target_duration_seconds` 仅是用户选择的生成目标和模型提示参数，不是模型估算结果或最终成片的强制时长。模型返回的场景估算总时长可以偏离该值。服务端取得每段真实 TTS 音频后不执行变速、补白或裁切，直接以原始 WAV 的实际整数毫秒时长建立时间轴；字幕、素材裁剪、预览、BGM 和最终视频都以所有 TTS 片段的实际时长总和为准。
